...
class FiniteType (f : X ⟶ Y) extends LocallyOfFiniteType f, QuasiCompact f

class Scheme.IsVariety {k : CommRingCat} [Field k] 
    (X : Over (specObj k)) 
  extends IsReduced X.left, Hom.IsSeparated X.hom, FiniteType X.hom

class Scheme.IsIntegralVariety {k : CommRingCat} [Field k] 
    (X : Over (specObj k))
  extends IsIntegral X.left, Hom.IsSeparated X.hom, FiniteType X.hom 
...
class IsProper (f : X ⟶ Y) extends 
  FiniteType f, UniversallyClosed f, Hom.IsSeparated f
